function run_filt_latitude(latitude, flag_transf, flag_plot, ftype, fdomain)
...
fina = strcat('Results_', latitude, '_tr', num2str(flag_transf), '_', ftype, '_', fdomain);
...
if strcmp(latitude,whoreg) %region 
noc = noc+1;     
...
if numel(union(freq_BIC,freq_NML))==0
    ...
    [BICo, SCo] = save_res(seven, fid, list_countries{i}, whoreg, freq_BIC, freq_NML, T0, delta, BICo, SCo);
end

if flag_plot==1
    label = strcat(list_countries{i}, ' (before filtering)');
    fig = plot_periodogram(1, 1, Phi_WP(1:length(vec_f)), vec_f, freq_BIC, freq_NML, label);
end
...
if sum( (freq_BIC>=min(Wn)).*(freq_BIC<=max(Wn)) ) + sum( (freq_NML>=min(Wn)).*(freq_NML<=max(Wn) ) )  >0  
    ...
    [BICf, SCf] = save_res(seven, fid, list_countries{i}, whoreg, freq_BIC_f, freq_NML_f, T0, delta, BICf, SCf);
    
    if flag_plot==1
        label = strcat(list_countries{i}, ' (after filtering)');
        plot_periodogram(1, 2, Phi_WP_f(1:length(vec_f)), vec_f, freq_BIC_f, freq_NML_f, label);
    end
...
end %if strcmp(latitude,whoreg)
    
end %for list_countries

save(strcat(fina,'.mat'),'noc','BICo','BICf','SCo','SCf','seven');
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function fig = plot_periodogram(fig, panel, Periodogram, Freq, freq_BIC, freq_SC, label)
...
if numel(freq_SC)>0
    ...
    if numel(freq_BIC)>0
        legend('Periodogram','BIC','SC','Location','southwest');
    else
        legend('Periodogram','SC','Location','southwest');
    end
    hold on
end

xlim([0 1/2]);
xl = strcat('Frequency [cycles/', 'week', ']');
xlabel(xl);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [BIC, SC] = save_res(seven, fid, country, whoreg, freq_BIC, freq_NML, T0, delta, BIC, SC)
